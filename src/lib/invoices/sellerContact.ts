/** Vienoje eilutėje saugomas kontaktas (PDF antraštė / legacy). */
...
/** Skaido „el. paštas · telefonas“ arba vieną reikšmę. */
...
export function sellerEmailPhoneFromRow(
  contactLine: string,
  emailCol: string | null | undefined,
  phoneCol: string | null | undefined
): { email: string; phone: string } {
  const e = (emailCol ?? "").trim();
  const p = (phoneCol ?? "").trim();
  if (e || p) return { email: e, phone: p };
  return splitCombinedSellerContactLine(contactLine);
}
